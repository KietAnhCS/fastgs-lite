[← Mục lục](00-muc-luc.md) · Chương 14/15

# Chương 14 — Hướng dẫn Triển khai Thực tế: Colab T4 & Nhật ký Huấn luyện

> Nguồn: `DOCS/colab-t4-guide.md` (toàn văn), `DOCS/history-train.md` (toàn văn)
...
## 14.2 Nhật ký huấn luyện thật (history-train.md)
...
**Bài tập 14.1.** `scene/gaussian_model.py:190` (`optimizer_step`) chia vòng đời training thành ba khoảng theo dòng 227 và 233: `iteration <= 15000`, `15000 < iteration <= 20000`, `iteration > 20000`. Giải thích tại sao mục 2.1 kết luận "đừng hạ `Config.iterations` xuống 15k khi muốn kết quả đầy đủ", dựa trên những gì thực sự xảy ra ở đoạn 15k–30k (tần suất bước Adam, và lời gọi `final_prune_fastgs` mỗi 3000 vòng ở `pipeline/trainer.py:125-128`).
...
**Bài tập 14.5.** Trace theo bảng ở mục 5 (Chống tràn RAM): liệt kê đúng thứ tự các bước dọn bộ nhớ xảy ra "Ngay sau `train_scene`" (`pipeline/trainer.py:203-208`) và "Sau khi render xong mỗi scene" (`pipeline/run.py:61`), rồi giải thích vì sao đóng gói `submission.zip` dùng `zipfile.ZIP_STORED` ghi thẳng ra đĩa thay vì `shutil.make_archive` qua `BytesIO`. Liên hệ với con số RAM đo được ở Phiên 1 (9.58/12.7 GB tại 3 cảnh cuối, mục 1.6) để giải thích vì sao thiết kế này quan trọng trên Colab free.

**Bài tập 14.6.** Bảng mục 1.4 cho thấy Score live trung bình 0.8769 và Score cuối (báo cáo) trung bình 0.7643 — chênh 0.1126. Liệt kê đủ ba nguyên nhân gây ra khoảng cách này (mạng LPIPS, độ phân giải, số view) và giải thích tại sao mục 1.4 kết luận "khi so sánh giữa các phiên, chỉ so live với live, cuối với cuối".

**Bài tập 14.7.** `densification_interval = 100` (mặc định gốc của `arguments/__init__.py:83`) gọi `compute_gaussian_score_fastgs` **144** lần trong khoảng 0–15000 vòng, còn `densification_interval = 500` (giá trị `Config` mặc định) chỉ gọi **28** lần. Mỗi lần gọi lấy mẫu 10 camera và render hai lượt mỗi camera (mục 2.3). Ước lượng tỉ lệ số lần render phụ trội giữa hai cấu hình, và giải thích vì sao mục 2.3 xếp đây là "lever thời gian lớn thứ hai sau độ phân giải".

---

[← Chương 13](13-tong-hop-chi-phi-fastgs.md) | [Mục lục](00-muc-luc.md) | [Chương 15 →](15-phu-luc-kiem-dinh-tai-nguyen.md)
