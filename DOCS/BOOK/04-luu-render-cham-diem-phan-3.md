...
3. **`cams, pose_info = testposes.load_test_cameras(cfg, scene)`** (`pipeline/testposes.py:62`, gọi ngay sau khi dựng `GaussianModel`) — đây là **nhánh camera thật của cuộc thi**, đọc trực tiếp `test/test_poses.csv` do ban tổ chức cung cấp (nếu tồn tại), qua `poses_csv`/`read_rows`. Từ đây có hai nhánh loại trừ nhau:
   - **`cams is not None`** (có `test_poses.csv` — đúng layout cuộc thi VAI_NVS_DATA_ROUND2, đã dùng thật cho scene `HCM0539`): gọi thẳng `gaussians.load_ply(...)` trên file `point_cloud/iteration_<n>/point_cloud.ply`, **không dựng `Scene`** — tiết kiệm cả việc đọc lại COLMAP lẫn nạp 240 ảnh train vào VRAM chỉ để lấy camera test.
   - **`cams is None`** (không có CSV — layout tandt_db/benchmark cũ): quay về hành vi gốc, dựng `Scene(dataset, gaussians, load_iteration=iterations, shuffle=False, camera_sets=("test",))` rồi `cams = sorted(scene_obj.getTestCameras(), key=lambda c: c.image_name)` — chỉ nạp tập `test`, không nạp `train`, vì lượt render này không cần tới ảnh train.
   
   Cả hai nhánh đều nạp đúng `.ply` tại `iteration_<iterations>` (cơ chế `load_ply` ở §38); nhánh CSV không cần `shuffle=False` vì không dựng `Scene`.
4. **`background`** dựng từ `dataset.white_background`, giống mọi nơi khác trong repo.
5. **Thứ tự file nộp bài**: nhánh CSV giữ nguyên thứ tự dòng trong `test_poses.csv` khi `cfg.submission_order="csv"` (mặc định — đúng chuẩn "đánh số theo thứ tự dòng file ban tổ chức"), hoặc sắp theo `image_name` nếu `submission_order="name"`; nhánh không-CSV luôn sắp theo **tên ảnh gốc** (`key=lambda c: c.image_name`) để đảm bảo thứ tự **ổn định và tái lập được** giữa các lần chạy.
...
6. **Đối chiếu trực tiếp với `test_poses.csv`** (`pipeline/submission.py:169-178`, thêm ở đợt tích hợp layout cuộc thi — mục dưới đây từng bị mô tả sai là "không thể kiểm tra" trong một bản tài liệu cũ hơn): với mỗi scene, nếu `testposes.poses_csv(cfg, scene)` tồn tại, đọc `want = testposes.read_rows(csv_path)` rồi so **số ảnh** (`len(files) != len(want)` → `"{scene}: {len(files)} ảnh nhưng CSV có {len(want)} pose"`) và **kích thước ảnh đầu tiên trong zip** với `width`/`height` khai báo trong CSV (`(width, height) != (want_w, want_h)` → báo lệch). Đây chính là bước xác nhận **có thẩm quyền** với layout `VAI_NVS_DATA_ROUND2`, vì `test_poses.csv` do ban tổ chức phát hành, không phải suy đoán từ `llffhold`.
7. In `"OK: submission hợp lệ"` nếu `problems` rỗng, ngược lại in `"CẢNH BÁO:\n- " + "\n- ".join(problems)"`.

**Những gì `verify()` vẫn KHÔNG thể kiểm tra** — kể cả khi có `test_poses.csv`, cần nói thẳng để tránh ảo tưởng "verify OK = chắc chắn được điểm cao":

- Khi **không có** `test_poses.csv` (layout benchmark cũ kiểu tandt_db), bước 6 ở trên bị bỏ qua hoàn toàn — quay về đúng giới hạn mô tả gốc: chỉ so với `expected` (danh sách scene) do người dùng tự truyền, không biết số lượng/độ phân giải test-pose thật.
...
- Nó không biết **điểm số chính thức** ban tổ chức sẽ chấm (PSNR/SSIM/LPIPS trên ground-truth riêng của họ) — cột `score/psnr/ssim/lpips` trong `leaderboard.csv` chỉ là điểm tự chấm nội bộ, có thể lệch với điểm chính thức.

**Số đo thật, lần chạy `HCM0539`** (thư mục `output/new249`, notebook `bts-digital-twin.ipynb`, 30000 iteration): `verify()` đối chiếu `test_poses.csv` báo **60/60 ảnh khớp**, kích thước ảnh **1320×989** đúng như CSV yêu cầu (`submission_resolution=1` → render đúng độ phân giải gốc, khác `resolution=2` lúc train), tên file liên tục `0001.png…0060.png`, không thiếu/thừa scene → in `"OK: submission hợp lệ"`. `leaderboard.csv` cùng lần chạy: `live_score=0.8579`, `live_psnr=25.268` (`psnr_norm=0.8423`), `live_ssim=0.8659`, `live_lpips=0.1364`, `344,484` Gaussian cuối, `1776.2 s` train, **VRAM đỉnh 1.92 GB** trên T4 (14.56 GB khả dụng — dư nhiều, xem §45/§47 về ngưỡng OOM). Đây là bằng chứng thực nghiệm cả hai nhánh mới ở §39 (đọc `test_poses.csv`, không nạp ảnh train khi render submission) lẫn bước đối chiếu CSV ở `verify()` hoạt động đúng trên dữ liệu cuộc thi thật, không chỉ trên benchmark `tandt_db` cũ.

- Tóm lại, `verify()` đảm bảo **tính nhất quán nội bộ** của file zip (tên liên tục, đúng cấu trúc thư mục, không thiếu/thừa scene) **và**, khi có `test_poses.csv`, đối chiếu được số lượng/kích thước ảnh với chính bộ pose ban tổ chức phát hành — nhưng vẫn không phải một bộ chấm điểm giả lập cho điểm số chính thức.
...
|---|---|---|---|
| SH degree tối đa | 3 | `arguments/__init__.py:48` (`ModelParams.sh_degree`) | Bậc cầu điều hoà tối đa cho màu phụ thuộc góc nhìn |
...
| Importance score | **Số pixel bị đánh dấu lỗi mà một Gaussian phủ lên, tính trung bình trên 10 camera lấy mẫu, làm tròn xuống.** Dùng để lọc ứng viên densify (`metric_mask = importance_score > 5`). ⚠ Hai hiểu lầm phải tránh: (a) **đơn vị là pixel**, không phải số camera "bỏ phiếu" — kernel cộng `+1` mỗi pixel lỗi chứ không phải mỗi view; (b) điểm **cao** nghĩa là Gaussian đó đang nằm dưới vùng render **sai**, tức "đáng để thêm chi tiết vào đây", chứ không phải "Gaussian này tốt/quan trọng". Xem PIPELINE-2 §28 |
